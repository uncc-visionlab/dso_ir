    ...
    namespace IOWrap {
        ...
        void PangolinDSOViewer::run() {
            ...
            while (!pangolin::ShouldQuit() && running) {
                ...
                glClearColor(1, 1, 1, 1); // set screen color
                ...
                if (settings_savePCButton.Get()) {
                    ...
                    while (1) {
                        if (isPCLfileClose) {
                            if (boost::filesystem::exists(strTmpFileName)) {
                                std::ofstream finalFile(strSaveFileName);
                                finalFile << std::string("# .PCD v0.7 - Point Cloud Data file format\n");
                                finalFile << std::string("VERSION 0.7\n");
                                finalFile << std::string("FIELDS x y z rgb\n");
                                finalFile << std::string("SIZE 4 4 4 4\n");
                                finalFile << std::string("TYPE F F F U\n");
                                finalFile << std::string("COUNT 1 1 1 1\n");
                                ...
                                printf("PCD File 'pointCloud_dso.pcd' is saved.\n");

                                remove(strTmpFileName.c_str());

                                break;
                            } else {
                                printf("Saving failed. PCD File 'pointCloud_dso.pcd' already exists.\n");
                                break;
                            }
                        }
...
//            if (file.is_open())
//                file << lastNMappingMs.back() << std::endl;
