    ...
    void EnergyFunctional::setAdjointsF(CalibHessian *Hcalib) {
        ...
        for (int h = 0; h < nFrames; h++)
            for (int t = 0; t < nFrames; t++) {
                ...
                FrameHessian *hostPrev = prevFrames[h];
                FrameHessian *target = frames[t]->data;
                FrameHessian *targetPrev = prevFrames[t];
                ...
                Vec4f affLL = AffLight::fromToVecExposure(host->ab_exposure, target->ab_exposure, hostPrev->ab_exposure, targetPrev->ab_exposure,
                                                          host->aff_g2l_0(), target->aff_g2l_0(), IR_Data).cast<float>();
                //std::cout << "host->aff_g2l_0().a = " << host->aff_g2l_0().a << "; host->aff_g2l_0().b = " << host->aff_g2l_0().b << std::endl;
                //std::cout << "affLL[0] = " << affLL[0] << "; affLL[1] = " << affLL[1] << std::endl;
                ...
                if (!IR_Data) {
                    ...
                    if (ir_correction_model == 0) {
                        // ideal model
                        AT(6, 6) = -(1/affLL[2]) * (affLL[0] - 1) * (-target->ab_exposure);
                        AH(6, 6) = (affLL[0]/(affLL[2]*affLL[2])) * (affLL[2] - 1) * (-host->ab_exposure);
                        AT(7, 7) = -(affLL[1] * (-(setting_frameTime - targetPrev->ab_exposure)));
                        AH(7, 7) = (affLL[0]/affLL[2]) * affLL[3] * (-(setting_frameTime - hostPrev->ab_exposure));
                        //std::cout << "affLL[0] = " << affLL[0] << "; affLL[1] = " << affLL[1] << "; affLL[2] = " << affLL[2] << "; affLL[3] = " << affLL[3] << std::endl;
                        //std::cout << "AT66 = " << AT(6, 6) << "; AH66 = " << AH(6, 6) << "; AT77 = " << AT(7, 7) << "; AH77 = " << AH(7, 7) << std::endl;
                    }
                    else if (ir_correction_model == 1) {
                        // non-linear exponential model
                        AT(6, 6) = -1;
                        AH(6, 6) = 1;   // I thought this should be 0 based on a'(ai) however 0 will cause a non-convergent problem
                        AT(7, 7) = -affLL[1] * (setting_frameTime - targetPrev->ab_exposure);
                        AH(7, 7) = affLL[3] * (setting_frameTime - hostPrev->ab_exposure);  // I thought this should be 0 based on b'(bi)
                    }
                    else {
...
// calculate H and b for non linearized active points (mode = 0)
...
// accumulates & shifts L.
// calculate H and b for linearized active points (mode = 1). It seems that the points that are satisfied the condition to be added
// do not even exist as they were deleted in marginalization 
    void EnergyFunctional::accumulateLF_MT(MatXX &H, VecX &b, bool MT) {
        if (MT) {
            red->reduce(boost::bind(&AccumulatedTopHessianSSE::setZero, accSSE_top_L, nFrames, _1, _2, _3, _4), 0, 0,
                        0);
...
// Schur Completement
    ...
    void EnergyFunctional::resubstituteF_MT(VecX x, CalibHessian *HCalib, bool MT) {
        ...
        HCalib->step = -x.head<CPARS>();    // camera parameters increment

        Mat18f *xAd = new Mat18f[nFrames * nFrames];
        VecCf cstep = xF.head<CPARS>();
        for (EFFrame *h: frames) {
            h->data->step.head<8>() = -x.segment<8>(CPARS + 8 * h->idx);    // pose and intensity transform parameters increment
    ...
    EFFrame *EnergyFunctional::insertFrame(FrameHessian *fh, FrameHessian *fhPrev, CalibHessian *Hcalib) {  // JZ modification: previous frame
        EFFrame *eff = new EFFrame(fh);
        eff->idx = frames.size();
        frames.push_back(eff);
        // JZ modification: previous frame
        prevFrames.push_back(fhPrev);
...
//	std::cout << std::setprecision(16) << "SVecI: " << SVecI.transpose() << "\n\n";
        ...
        // JZ modification: previous frame
        assert(frames.size() == prevFrames.size());
...
//	std::cout << "EigPost:: " << eigenvaluesPost.transpose() << "\n";
    ...
    void EnergyFunctional::solveSystemF(int iteration, double lambda, CalibHessian *HCalib) {
        ...
        resubstituteF_MT(x, HCalib, multiThreading);    // In this function, the first 8 values in x will be assigned to steps of each frame
        currentLambda = 0;
