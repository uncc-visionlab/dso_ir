...
namespace dso {
    ...
    float setting_frameTime = 33; // (in ms)
    // JZ modification: AffLight (gradient)
    float init_A = 0.25;   // for ideal IR-correction model
    float init_B = 0.05; // for ideal IR-correction model
    bool IR_Data = true;
    int ir_correction_model = 0;    // 0: ideal model; 1: non-linear exponential model; 2: linear model
...
// Added By Yo Han.
// https://github.com/Neoplanetz/dso_with_saving_pcl/blob/master/src/util/settings.cpp
    int numPCL = 0;
    bool isSavePCL = false;
    bool isWritePCL = false;
    bool isPCLfileClose = false;
    std::string strTmpFileName = "pointCloudTemp.pcd";
    std::string strSaveFileName = "pointCloud_dso.pcd";
