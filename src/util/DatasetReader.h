...
//    ImageAndExposure *getImageIR(int id, bool forceLoadDirectly = false) {
//        return getImage_internalIR(id, 0);
//    }
...
//    MinimalImageB16 *getImageRawIR_internal(int id, int unused) {
//        if (!isZipped) {
//            // CHANGE FOR ZIP FILE
...
//    ImageAndExposure *getImage_internalIR(int id, int unused) {
//        MinimalImageB16 *minimg = getImageRawIR_internal(id, 0);
...
//    }
    ...
    inline void loadTimestamps() {
        std::ifstream tr;
        std::string timesFile = path.substr(0, path.find_last_of('/')) + "/times";
        std::size_t found = path.find("_pt1");
        if (!IR_Data) {
            timesFile = timesFile + "_rgb";
        }
        else {
            timesFile = timesFile + "_ir";
        }
        if (found != std::string::npos) {
            timesFile = timesFile + path.substr(found);
            timesFile = timesFile.substr(0, timesFile.find_last_of('.'))+ ".txt";
        }
        else {
            timesFile = timesFile + ".txt";
        }
        std::cout << "TimeStamp filepath is " << timesFile << std::endl;
        ...
        while (!tr.eof() && tr.good()) {
            ...
            } else if (2 == sscanf(buf, "%s %lf", id, &stamp)) {
                timestamps.push_back(stamp);
                exposures.push_back(exposure);
            }
        }
