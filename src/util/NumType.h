...
// JZ modification: affLight (res)
    struct AffLight {
        AffLight(float a, float b) : a(a), b(b) {};

        AffLight() : a(0), b(0) {};

        // Affine Parameters:
        float a, b;    // I_frame = exp(a)*I_global + b. // I_global = exp(-a)*(I_frame - b).

        static Vec4 fromToVecExposure(float exposureF, float exposureT, float exposureFPrev, float exposureTPrev, AffLight g2F,
                                      AffLight g2T, bool IRData)     // Here the exposureF and exposureT are exposure time of the previous frames
        {
            if (exposureF == 0 || exposureT == 0) {
                exposureT = exposureF = 1;
                //printf("got exposure value of 0! please choose the correct model.\n");
            ...
            float a, b, a1, b1;
            if (!IRData) {
                //exposureT = exposureF = 15;
                a = expf(g2T.a - g2F.a) * exposureT / exposureF;
                b = g2T.b - a * g2F.b;
                a1 = 0;
                b1 = 0; // a1 and b1 will not be used in the non-IR correction model
            }
            else {
                if (ir_correction_model == 0) {
                    // ideal model
                    a = (1 - expf(-(g2T.a + init_A) * exposureT)) ;// / (1 - expf(-g2F.a * exposureF));
                    b = expf(-(g2T.b + init_B)* (setting_frameTime - exposureTPrev));
                    a1 = (1 - expf(-(g2F.a + init_A) * exposureF));
                    b1 = expf(-(g2F.b + init_B) * (setting_frameTime - exposureFPrev));
                }
                else if (ir_correction_model == 1) {
                    // non-linear exponential model
                    a = g2T.a + init_A; // - g2F.a;
                    b = expf((g2T.b + init_B) * (setting_frameTime - exposureTPrev)); // - expf((g2F.b) * (setting_frameTime - exposureFPrev));
                    a1 = g2F.a + init_A;
                    b1 = expf((g2F.b + init_B) * (setting_frameTime - exposureFPrev));
                }
                else {
