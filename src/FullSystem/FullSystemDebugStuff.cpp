...
namespace dso {
    ...
    void FullSystem::debugPlotTracking() {
        ...
        for (int m = 0; m < frameHessians.size(); m++) {
            ...
            //for (FrameHessian *f2: frameHessians) {
            for (int n = 0; n < frameHessians.size(); n++) {
                FrameHessian *f2 = frameHessians[n];
                assert(frameHessians.size() == frameHessiansPrev.size());
                FrameHessian *f2Prev = frameHessiansPrev[n];
                MinimalImageB3 *debugImage = f2->debugImage;
                images.push_back(debugImage);

                Eigen::Vector3f *fd = f2->dI;

                // JZ modification: affLight (for displaying)
                Vec4 affL = AffLight::fromToVecExposure(f2->ab_exposure, f->ab_exposure, f2Prev->ab_exposure, fPrev->ab_exposure, f2->aff_g2l(), f->aff_g2l(), IR_Data);
                ...
                for (int i = 0; i < wh; i++) {
                    ...
                    float colL;
                    if (!IR_Data) {
                        colL = affL[0] * fd[i][0] + affL[1];
                    }
                    else {
                        if (ir_correction_model == 0) {
                            // ideal model
                            colL = (fd[i][0] - affL[1] * f2Prev->dI[i][0]) / affL[0];
                        }
                        else if (ir_correction_model == 1) {
                            // non-linear exponential model
                            colL = fd[i][0] - affL[0] - f2Prev->dI[i][0] * affL[1];
                        }
                    }
                    if (colL < 0) colL = 0;
