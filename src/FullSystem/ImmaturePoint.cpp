...
// JZ modification: previous frame (colorPrev)
    ImmaturePoint::ImmaturePoint(int u_, int v_, FrameHessian *host_, FrameHessian *hostPrev, float type,
                                 CalibHessian *HCalib)
        ...
        for (int idx = 0; idx < patternNum; idx++) {
            ...
            // JZ modification: previous frame (colorPrev)
            if (hostPrev != NULL) {
                Vec3f ptcPrev = getInterpolatedElement33BiLin(hostPrev->dI, u + dx, v + dy, wG[0]);
                colorPrev[idx] = ptcPrev[0];
            }
            if (!std::isfinite(color[idx]) || !std::isfinite(colorPrev[idx])) {
                energyTH = NAN;
                return;
            }
        ...
        idepth_GT = 0;
        quality = 10000;
        // JZ modification: previous frame (previous exposure)
        prev_ab_exposure = hostPrev->ab_exposure;
    }
...
// JZ modification: previous frame
    ImmaturePointStatus
    ImmaturePoint::traceOn(FrameHessian *framePrev, FrameHessian *frame, const Mat33f &hostToFrame_KRKi,
                           const Vec3f &hostToFrame_Kt, const Vec4f &hostToFrame_affine, CalibHessian *HCalib,
                           bool debugPrint) {
...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
        ...
        for (int i = 0; i < numSteps; i++) {
            ...
            for (int idx = 0; idx < patternNum; idx++) {
                ...
                float hitColorPrev = getInterpolatedElement31(framePrev->dI,
                                                              (float) (ptx + rotatetPattern[idx][0]),
                                                              (float) (pty + rotatetPattern[idx][1]),
                                                              wG[0]);

                if (!std::isfinite(hitColor) || !std::isfinite(hitColorPrev) || !std::isfinite(color[idx]) ||
                    !std::isfinite(colorPrev[idx])) {
                    energy += 1e5;
                    continue;
                }
                ...
                float residual;
                if (!IR_Data) {
                    residual = hitColor - (float)(hostToFrame_affine[0] * color[idx] + hostToFrame_affine[1]);
                }
                else {
                    if (ir_correction_model == 0) {
                        // ideal model
                        float prevValue = (colorPrev[idx] + hitColorPrev) / 2;
                        residual = hitColor - (hostToFrame_affine[0] / hostToFrame_affine[2]) * color[idx] + (hostToFrame_affine[0] / hostToFrame_affine[2]) * hostToFrame_affine[3] * colorPrev[idx] - hostToFrame_affine[1] * colorPrev[idx];//hitColorPrev;
                    }
                    else if (ir_correction_model == 1) {
                        // non-linear exponential model
                        residual = hitColor - color[idx] - (hostToFrame_affine[0] - hostToFrame_affine[2]) - (hostToFrame_affine[1] - hostToFrame_affine[3]) * colorPrev[idx];
                    }
                    else {
                        float hitColorPrevCoolTime = setting_frameTime - framePrev->ab_exposure;
                        float refColorPrevCoolTime = setting_frameTime - prev_ab_exposure;
                        float hitColorPrevRes = hitColorPrev * pow(hostToFrame_affine[1], hitColorPrevCoolTime);
                        float refColorPrevRes = colorPrev[idx] * pow(hostToFrame_affine[1], refColorPrevCoolTime);
                    }
                }
        ...
        for (int it = 0; it < setting_trace_GNIterations; it++) {
            ...
            for (int idx = 0; idx < patternNum; idx++) {
                ...
                Vec3f hitColorPrev = getInterpolatedElement33(framePrev->dI,
                                                              (float) (bestU + rotatetPattern[idx][0]),
                                                              (float) (bestV + rotatetPattern[idx][1]), wG[0]);

                if (!std::isfinite((float) hitColor[0]) || !std::isfinite((float) hitColorPrev[0]) ||
                    !std::isfinite(color[idx]) || !std::isfinite(colorPrev[idx])) {
                    energy += 1e5;
                    continue;
                }
                // JZ modification: previous frame (res)
                ...
                float residual;
                if (!IR_Data) {
                    residual = hitColor[0] - (float) (hostToFrame_affine[0] * color[idx] + hostToFrame_affine[1]);
                }
                else {
                    if (ir_correction_model == 0) {
                        // ideal model
                        float prevValue = (colorPrev[idx] + hitColorPrev[0]) / 2;
                        residual = hitColor[0] - (hostToFrame_affine[0] / hostToFrame_affine[2]) * color[idx] + (hostToFrame_affine[0] / hostToFrame_affine[2]) * hostToFrame_affine[3] * colorPrev[idx] - hostToFrame_affine[1] * colorPrev[idx];//hitColorPrev[0];
                    }
                    else if (ir_correction_model == 1) {
                        // non-linear exponential model
                        residual = hitColor[0] - color[idx] - (hostToFrame_affine[0] - hostToFrame_affine[2]) - (hostToFrame_affine[1] - hostToFrame_affine[3]) * colorPrev[idx];
                    }
                    else {
                        float hitColorPrevCoolTime = setting_frameTime - framePrev->ab_exposure;
                        float refColorPrevCoolTime = setting_frameTime - prev_ab_exposure;
                        float hitColorPrevRes = hitColorPrev[0] * pow(hostToFrame_affine[1], hitColorPrevCoolTime);
                        float refColorPrevRes = colorPrev[idx] * pow(hostToFrame_affine[1], refColorPrevCoolTime);
                    }
                }
...
//			 && absGrad2*areaGradientSlackFactor < host->frameGradTH*0.50f))
    ...
    float ImmaturePoint::getdPixdd(     // this function is not being used
            CalibHessian *HCalib,
    ...
    float ImmaturePoint::calcResidual(          // this function is not being used
        ...
        // JZ modification: affLight
        Vec4f affLL = precalc->PRE_aff_mode;
        ...
        for (int idx = 0; idx < patternNum; idx++) {
            ...
            float residual =
                    hitColor[0] - (affLL[0] * color[idx] + affLL[1]);  // the entire function is not being used anywhere
    ...
    double ImmaturePoint::linearizeResidual(
        ...
        const Eigen::Vector3f *dIl = tmpRes->target->dI;
        // JZ modification: previous frame
        const Eigen::Vector3f *dIlPrev = tmpRes->targetPrev->dI;
        ...
        for (int idx = 0; idx < patternNum; idx++) {
            ...
            Vec3f hitColor = (getInterpolatedElement33(dIl, Ku, Kv, wG[0]));
            Vec3f hitColorPrev = (getInterpolatedElement33(dIlPrev, Ku, Kv, wG[0]));
            ...
            float residual;
            if (!IR_Data) {
                residual = hitColor[0] - (affLL[0] * color[idx] + affLL[1]);
            }
            else {
                if (ir_correction_model == 0) {
                    // ideal model
                    float prevValue = (colorPrev[idx] + hitColorPrev[0]) / 2;
                    residual = hitColor[0] - (affLL[0] / affLL[2]) * color[idx] + (affLL[0] / affLL[2]) * affLL[3] * colorPrev[idx] - affLL[1] * colorPrev[idx];//hitColorPrev[0];
                }
                else if (ir_correction_model == 1) {
                    // non-linear exponential model
                    residual = hitColor[0] - color[idx] - (affLL[0] - affLL[2]) - (affLL[1] - affLL[3]) * colorPrev[idx];
                }
                else {
                    float hitColorPrevCoolTime = setting_frameTime - tmpRes->targetPrev->ab_exposure;
                    float refColorPrevCoolTime = setting_frameTime - prev_ab_exposure;
                    float hitColorPrevRes = hitColorPrev[0] * pow(affLL[1], hitColorPrevCoolTime);
                    float refColorPrevRes = colorPrev[idx] * pow(affLL[1], refColorPrevCoolTime);
                }
            }
