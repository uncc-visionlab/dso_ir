...
namespace dso {
    ...
    PointHessian *FullSystem::optimizeImmaturePoint(
        ...
        for (unsigned int i = 0; i < frameHessians.size(); i++) {
            ...
            FrameHessian *fhLast = frameHessiansPrev[i];
            if (fh != point->host)   // All active frames except for the current keyframe
            {
                ...
                residuals[nres].targetPrev = fhLast;
                nres++;
            }
        }
