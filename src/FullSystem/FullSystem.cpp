...
namespace dso {
    ...
    FullSystem::~FullSystem() {
        ...
        for (FrameHessian *fh: unmappedTrackedFrames)
            delete fh;
        // JZ modification: previosu frame
        for (FrameHessian *fhPrev: unmappedTrackedPreviousFrames)
            delete fhPrev;
...
// JZ modification: previous frame
    Vec4 FullSystem::trackNewCoarse(FrameHessian *fhPrev, FrameHessian *fh) {
        ...
        if (allFrameHistory.size() == 2)
            ...
            FrameShell *slast = allFrameHistory[allFrameHistory.size() - 2];  // previous frame
            FrameShell *sprelast = allFrameHistory[allFrameHistory.size() - 3];   // the frame before previous frame
        ...
        for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
            ...
            bool trackingIsGood = coarseTracker->trackNewestCoarse(
                    fhPrev, fh, lastF_2_fh_this, aff_g2l_this,
                    pyrLevelsUsed - 1,
        ...
        // JZ modification: affLight
        fh->shell->aff_g2l = aff_g2l;   // aff_g2l gets updated after a successful tracking
        ...
        if (!setting_debugout_runquiet)
            // JZ modification: affLight
            //printf("Coarse Tracker tracked ab = %f %f (exp time %f). Res %f!\n", aff_g2l.a, aff_g2l.b, fh->ab_exposure, achievedRes[0]);
...
// Use the new incoming frame (keyframe or non-keyframe) to update the status of the immature points in the active keyframes
// JZ modification: previous frame
    void FullSystem::traceNewCoarse(FrameHessian *fhPrev, FrameHessian *fh) {
        ...
        printf("Calibrated camera parameters are fx = %f, fy = %f, cx = %f, cy = %f.\n", K(0, 0), K(1, 1),
               K(0, 2), K(1, 2));
        //JZ modification: previous frame (frameHessiansPrev)
        //for(FrameHessian* host : frameHessians)		// go through all active frames
        assert(frameHessians.size() == frameHessiansPrev.size());
        for (int i = 0; i < frameHessians.size(); i++) {
            FrameHessian *host = frameHessians[i];
            FrameHessian *hostPrev = frameHessiansPrev[i];
            SE3 hostToNew = fh->PRE_worldToCam * host->PRE_camToWorld;
            Mat33f KRKi = K * hostToNew.rotationMatrix().cast<float>() * K.inverse();
            Vec3f Kt = K * hostToNew.translation().cast<float>();
            // JZ modification: affLight
            //Vec2f aff = AffLight::fromToVecExposure(host->ab_exposure, fh->ab_exposure, host->aff_g2l(), fh->aff_g2l()).cast<float>();
            Vec4f aff = AffLight::fromToVecExposure(host->ab_exposure, fh->ab_exposure, hostPrev->ab_exposure, fhPrev->ab_exposure,
                                                    host->aff_g2l(), fh->aff_g2l(), IR_Data).cast<float>();
            if (frameHessians.size() == 1) {
                assert(host->immaturePoints.empty());     // first keyframe has ZERO immature points
            }
            for (ImmaturePoint *ph: host->immaturePoints) {
                // JZ modification: previous frame
                ph->traceOn(fhPrev, fh, KRKi, Kt, aff, &Hcalib, false);
...
//			trace_uninitialized, 100*trace_uninitialized/(float)trace_total);
    ...
    void FullSystem::activatePointsMT_Reductor(
        ...
        for (int k = min; k < max; k++) {
            // Optimize immature points in the ACTIVE frames
    ...
    void FullSystem::activatePointsMT() {
        ...
        // process immature points: activate/skip/delete
        for (FrameHessian *host: frameHessians)        // go through all active frames
        {
...
//			(int)toOptimize.size(), immature_deleted, immature_notReady, immature_needMarg, immature_want, immature_margskip);
        ...
        // Optimize the immature points in all ACTIVE frames from last step to get pointHessian by projecting each point to other keyframes and finding a match
        if (multiThreading)
            treadReduce.reduce(
    ...
    void FullSystem::flagPointsForRemoval() {
        ...
        for (FrameHessian *host: frameHessians)        // go through all active frames
        {
                ...
                if (ph->idepth_scaled < 0 || ph->residuals.size() == 0) {
                    ...
                    if (ph->isInlierNew()) {
                        ...
                        for (PointFrameResidual *r: ph->residuals) {
                            ...
                            r->applyRes(true);
                            if (r->efResidual->isActive())   // if the residual is active (can be used for optimization), then fix it and pass to the energy function
                            {
            ...
            if (dumpMeasures) {
                ...
                if (file0.is_open())
                    file0 << host->shell->incoming_id << std::endl;

                std::string filename2("marginalized_points.txt");
                std::fstream file2;

                file2.open(filename2, std::ios_base::app | std::ios_base::in);

                if (file2.is_open())
                    file2 << host->pointHessiansMarginalized.size() << std::endl;


                std::string filename3("outlier_points.txt");
                std::fstream file3;

                file3.open(filename3, std::ios_base::app | std::ios_base::in);

                if (file3.is_open())
                    file3 << host->pointHessiansOut.size() << std::endl;

                std::string filename1("active_points.txt");
                std::fstream file1;

                file1.open(filename1, std::ios_base::app | std::ios_base::in);

                if (file1.is_open())
                    file1 << host->pointHessians.size() << std::endl;
            }
...
// JZ modification: previous frame
    void FullSystem::addActiveFrame(ImageAndExposure *imagePrev, ImageAndExposure *image, int id) {
        ...
        int curr_active_pts = 0;
        for(FrameHessian *fh : frameHessians) {
            curr_active_pts += fh->pointHessians.size();
        }

        if (dumpMeasures) {
            std::string filename("rt_active_points.txt");
            std::fstream file;
            file.open(filename, std::ios_base::app | std::ios_base::in);
            if (file.is_open())
                file << curr_active_pts << std::endl;
        }
        ...
        // JZ modification: previous frame
        FrameHessian *fhPrev = new FrameHessian();
        if (imagePrev != nullptr) {
            fhPrev->ab_exposure = imagePrev->exposure_time;
            fhPrev->makeImages(imagePrev->image, &Hcalib);
        } else {
            fhPrev->ab_exposure = image->exposure_time;
            fhPrev->makeImages(image->image, &Hcalib);
        }
        if (!initialized) {
            // use initializer!
            if (coarseInitializer->frameID < 0)    // first frame set. fh is kept by coarseInitializer.
                // first frame will be added to the keyframe vector in initializeFromInitializer().
            {
                // JZ modification: previous frame
                coarseInitializer->setFirst(&Hcalib,
                                            fh, fhPrev);   // select points at the first frame. These points are the candidates for the second frame to match
                // to generate pointHessians and immaturePoints. These points are saved in CoarseInitializer::points
            } else if (coarseInitializer->trackFrame(fhPrev, fh, outputWrapper))    // if SNAPPED   // second frame
            {
                initializeFromInitializer(fhPrev,
                                          fh);      // set variable "initialized" to be True and set the previous frame (first frame) to be keyframe
                lock.unlock();
                deliverTrackedFrame(fhPrev, fh,
                                    true);      // make the second frame to be the KeyFrame and delivered to FullSystem::makeKeyFrame()
            } else {
            ...
            return;
        } else    // do front-end operation.      // third, fourth, ... frames
        {
            // =========================== SWAP tracking reference?. =========================
            // If tracker sees there is a new reference, tracker locks [coarseTrackerSwapMutex] and swaps the two
            // coarseTracker_forNewKF is set as reference and protected by [coarseTrackerSwapMutex]
            // coarseTracker is always used to track new frames and protected by [trackMutex]
            ...
            // this function will project all points in the new frame to the last keyframe and find a match. Those points may not correspond to immature points.
            Vec4 tres = trackNewCoarse(fhPrev, fh);
            ...
            if (setting_keyframesPerSecond > 0) {
                ...
                Vec4 refToFh = AffLight::fromToVecExposure(coarseTracker->lastRef->ab_exposure,
                                                           fh->ab_exposure, coarseTracker->lastRefPrev->ab_exposure, fhPrev->ab_exposure,
                                                           coarseTracker->lastRef_aff_g2l, fh->shell->aff_g2l, IR_Data);
                if (!IR_Data) {
                    // BRIGHTNESS CHECK
                    needToMakeKF = allFrameHistory.size() == 1 ||
                                   ...
                                   // JZ modification: affLight (comparison)
                    ...
                    needToMakeKF = allFrameHistory.size() == 1 ||
                                   ...
                                   setting_kfGlobalWeight * setting_maxShiftWeightR * sqrtf((double) tres[2]) / (wG[0] + hG[0]) +
                                   setting_kfGlobalWeight * setting_maxShiftWeightRT * sqrtf((double) tres[3]) / (wG[0] + hG[0]) +
                                   setting_kfGlobalWeight * setting_maxAffineWeight * fabs(logf((float) (refToFh[0] / refToFh[2]))) > 1 ||
                                   // JZ modification: affLight (comparison)
                                   2 * coarseTracker->firstCoarseRMSE < tres[0];
                }
            ...
            lock.unlock();
            // JZ modification: previous frame
            deliverTrackedFrame(fhPrev, fh, needToMakeKF);
            return;
        }
    }
    ...
    void FullSystem::reportTrackingTime(FrameHessian *fh) {
        ...
        std::string filename("DSO_tracking_time_ms.txt");
        std::fstream file;
        file.open(filename, std::ios_base::app | std::ios_base::in);
        if (file.is_open())
...
// JZ modification: previous frame
    void FullSystem::deliverTrackedFrame(FrameHessian *fhPrev, FrameHessian *fh, bool needKF) {


        if (linearizeOperation)      // this is always true if playbackSpeed = 0 in the main function
        {
            ...
            // JZ modification: previous frame
            if (needKF) makeKeyFrame(fhPrev, fh);
            else makeNonKeyFrame(fhPrev, fh);
        } else {
            boost::unique_lock<boost::mutex> lock(trackMapSyncMutex);
            unmappedTrackedFrames.push_back(fh);    // this will not be called if playbackSpeed = 0 in the main function
            // JZ modification: previous frame
            unmappedTrackedPreviousFrames.push_back(fhPrev);
    ...
    void FullSystem::mappingLoop() {
        ...
        while (runMapping) {
            ...
            // JZ modification: previous frame
            FrameHessian *fhPrev = unmappedTrackedPreviousFrames.front();
            ...
            if (unmappedTrackedFrames.size() >
                0) // if there are other frames to track, do that first.  // this will always be zero if playbackSpeed = 0 in the main function
            {
                lock.unlock();
                // JZ modification: previous frame
                makeNonKeyFrame(fhPrev, fh);
                lock.lock();
                ...
                if (needToKetchupMapping && unmappedTrackedFrames.size() > 0) {
                    ...
                    unmappedTrackedFrames.pop_front();
                    {
                        ...
                        fh->shell->camToWorld = fh->shell->trackingRef->camToWorld * fh->shell->camToTrackingRef;
                        // JZ modification: this will not be used if playbackSpeed = 0 in the main function (function won't get called)
                        fh->setEvalPT_scaled(fh->shell->camToWorld.inverse(), fh->shell->aff_g2l);
                    }
                    delete fh;  //  JZ modification: this will not be used if playbackSpeed = 0 in the main function (function won't get called)
                    // JZ modification
                    delete fhPrev;
                }
    ...
    void FullSystem::makeNonKeyFrame(FrameHessian *fhPrev, FrameHessian *fh) {
...
// JZ modification: previous frame
    void FullSystem::makeKeyFrame(FrameHessian *fhPrev, FrameHessian *fh) {
        ...
        // JZ modification: previous frame
        traceNewCoarse(fhPrev,
                       fh);     // fh doesn't have immature points yet (immature points will be calculated and added later in makeNetTraces() function)
        ...
        // JZ modification: previous frame (frameHessiansPrev)
        ef->insertFrame(fh, fhPrev, &Hcalib);
        frameHessiansPrev.push_back(fhPrev);    // fh is at least the second frame here thus fhPrev is not NULL
        setPrecalcValues();
        ...
        for (FrameHessian *fh1: frameHessians)        // go through all active frames
        {
            ...
            for (PointHessian *ph: fh1->pointHessians) {
                // JZ modification: previous frame
                PointFrameResidual *r = new PointFrameResidual(ph, fh1, fh, fhPrev);
        ...
        activatePointsMT();     // activate immature points in the keyframes
        ef->makeIDX();
...

        {
            ...
            FrameHessian * lastKfPrev = frameHessiansPrev.back();
            coarseTracker_forNewKF->setCoarseTrackingRef(frameHessians, lastKfPrev);
        ...
        // JZ modification: previous frame (colorPrev. modifications are inside the makeNewTraces())
        makeNewTraces(fhPrev, fh, 0);
        ...
        for (unsigned int i = 0; i < frameHessians.size(); i++) {
            ...
            if (frameHessians[i]->flaggedForMarginalization) {
                ...
                // JZ modification: previous frame (frameHessiansPrev)
                deleteOutOrder<FrameHessian>(frameHessiansPrev,
                                             frameHessiansPrev[i]);     // perform the same deletion for the frameHessians to the frameHessiansPrev
                i = 0;
            }
        }
...
// JZ modification: previous frame
    void FullSystem::initializeFromInitializer(FrameHessian *prevFrame, FrameHessian *newFrame) {
        boost::unique_lock<boost::mutex> lock(mapMutex);

        // add firstframe.
        FrameHessian *firstFrame = coarseInitializer->firstFrame;
        // JZ modification: previous frame
        FrameHessian *firstPrevFrame = coarseInitializer->firstPrevFrame;
        firstFrame->idx = frameHessians.size();
        frameHessians.push_back(firstFrame);
        frameHessiansPrev.push_back(firstPrevFrame);
        assert(frameHessians.size() == frameHessiansPrev.size());
        firstFrame->frameID = allKeyFramesHistory.size();
        allKeyFramesHistory.push_back(firstFrame->shell);
        // JZ modification: previous frame
        ef->insertFrame(firstFrame, firstPrevFrame,
                        &Hcalib);   // firstFrame has already been added to the prevFrameHessians[] in the addActive() function
        setPrecalcValues();
        ...
        for (int i = 0; i < coarseInitializer->numPoints[0]; i++) {
            ...
            // Generate immature points then convert them to PointHessian right away.
            // This indicates that the first frame does not have any immature points.
            // JZ modification: previous frame (colorPrev. fhPrev is a null pointer for the first frame)
            ImmaturePoint *pt = new ImmaturePoint(point->u + 0.5f, point->v + 0.5f, firstFrame, prevFrame,
        ...
        SE3 firstToNew = coarseInitializer->thisToNext;     // thisToNext was updated (calculated) in the trackFrame()
...
// JZ modification: previous frame
    void FullSystem::makeNewTraces(FrameHessian *prevFrame, FrameHessian *newFrame, float *gtDepth) {
        ...
        for (int y = patternPadding + 1; y < hG[0] - patternPadding - 2; y++)
            for (int x = patternPadding + 1; x < wG[0] - patternPadding - 2; x++) {
                ...
                // JZ modification: previous frame (colorPrev)
                // add colorPrev to the immature points
                ImmaturePoint *impt = new ImmaturePoint(x, y, newFrame, prevFrame, selectionMap[i], &Hcalib);
...
// calculate the pre-calculated parameters (KR, KT, aff, etc..) between frameHessians
    void FullSystem::setPrecalcValues() {
        // JZ modification: previous frame (frameHessiansPrev)
        //for(FrameHessian* fh : frameHessians)
        for (int k = 0; k < frameHessians.size(); k++) {
            FrameHessian *fh = frameHessians[k];
            FrameHessian *fhPrev = frameHessiansPrev[k];
            fh->targetPrecalc.resize(frameHessians.size());
            for (unsigned int i = 0; i < frameHessians.size(); i++) {
                fh->targetPrecalc[i].set(fh, fhPrev, frameHessians[i], frameHessiansPrev[i], &Hcalib);
            }
        }
