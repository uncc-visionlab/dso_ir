...
// JZ modification: previous frame
    bool CoarseInitializer::trackFrame(FrameHessian *previousFrameHessian, FrameHessian *newFrameHessian,
        ...
        if (firstFrame->ab_exposure > 0 && newFrame->ab_exposure > 0) {
            ...
            if (!IR_Data) {
                ...
                if (ir_correction_model == 0) {
                    // ideal model
                    A = 1 - expf(-(0 + init_A) * newFrame->ab_exposure);
                    B = expf(-(0 + init_B)* (setting_frameTime - prevFrame->ab_exposure));
                }
                ...
                refToNew_aff_current = AffLight(A, B);
            }
        }
...
// calculates residual, Hessian and Hessian-block needed for re-substituting depth.
    Vec3f CoarseInitializer::calcResAndGS(
            ...
            const SE3 &refToNew, AffLight refToNew_aff, //AffLight refToNew_aff, // JZ modification: affLight
            bool plot) {
        int wl = w[lvl], hl = h[lvl];
        Eigen::Vector3f *colorRef = firstFrame->dIp[lvl];
        Eigen::Vector3f *colorNew = newFrame->dIp[lvl];
        // JZ modification: previous frame
        Eigen::Vector3f *colorRefPrev = firstPrevFrame->dIp[lvl];
        Eigen::Vector3f *colorNewPrev = prevFrame->dIp[lvl];

        Mat33f RKi = (refToNew.rotationMatrix() * Ki[lvl]).cast<float>();
        Vec3f t = refToNew.translation().cast<float>();
        // JZ modification: affLight (gradient)
        Eigen::Vector4f r2new_aff;
        if (!IR_Data) {
            r2new_aff = Eigen::Vector4f(exp(refToNew_aff.a), refToNew_aff.b, 0, 0);
        }
        else {
            float A1, B1;
            if (ir_correction_model == 0) {
                A1 = 1 - expf(-(0 + init_A) * firstFrame->ab_exposure);
                B1 = expf(-(0 + init_B) * (setting_frameTime - firstPrevFrame->ab_exposure));
            }
            else if (ir_correction_model == 1) {
                A1 = 0 + init_A;
                B1 = expf((0 + init_B) * (setting_frameTime - firstPrevFrame->ab_exposure));
            ...
            r2new_aff = Eigen::Vector4f(refToNew_aff.a, refToNew_aff.b, A1, B1);
        }
        ...
        for (int i = 0; i < npts; i++) {
            ...
            for (int idx = 0; idx < patternNum; idx++) {
                ...
                Vec3f hitColorPrev = getInterpolatedElement33(colorNewPrev, Ku, Kv, wl);
                //float rlR = colorRef[point->u+dx + (point->v+dy) * wl][0];
                float rlR = getInterpolatedElement31(colorRef, point->u + dx, point->v + dy, wl);
                float rlRPrev = getInterpolatedElement31(colorRefPrev, point->u + dx, point->v + dy, wl);

                if (!std::isfinite(rlR) || !std::isfinite((float) hitColor[0]) ||
                    !std::isfinite((float) hitColorPrev[0]) || !std::isfinite((float) rlRPrev)) {
                    isGood = false;
                    break;
                }
                ...
                float residual;
                if (!IR_Data) {
                    residual = hitColor[0] - (r2new_aff[0] * rlR + r2new_aff[1]);
                }
                else {
                    // JZ modification: previous frame (res)
                    if (ir_correction_model == 0) {
                        // ideal model
                        float prevValue = (rlRPrev + hitColorPrev[0]) / 2;
                        residual = hitColor[0] - (r2new_aff[0] / r2new_aff[2]) * rlR + (r2new_aff[0] / r2new_aff[2]) * r2new_aff[3] * rlRPrev - r2new_aff[1] * rlRPrev;//
                        //std::cout << "hitColorPrev =  " << hitColorPrev[0] << "; colorPrev = " << rlRPrev << std::endl;
                    }
                    else if (ir_correction_model == 1) {
                        // non-linear exponential model
                        residual = hitColor[0] - rlR - (r2new_aff[0] - r2new_aff[2]) - (r2new_aff[1] - r2new_aff[3]) * rlRPrev;
                    }
                    else {
                        float hitColorPrevCoolTime = setting_frameTime - prevFrame->ab_exposure;
                        float refColorPrevCoolTime = setting_frameTime - firstFrame->ab_exposure;
                    }
                }
                ...
                // residual gradients w.r.t to pose at the new state j
                dp0[idx] = new_idepth * dxInterp; // dpi/pz' * dxfx
                dp1[idx] = new_idepth * dyInterp; // dpi/pz' * dyfy
                dp2[idx] =
                        -new_idepth * (u * dxInterp + v * dyInterp);   // -dpi/pz' * (px'/pz' * dxfx + py'/pz' * dyfy)
                ...
                // residual gradients w.r.t to brightness and thermal: I2 = exp(a) * I1 + b
                // JZ modification: affLight (gradient)
                if (!IR_Data) {
                    dp6[idx] = -hw * r2new_aff[0] * rlR; // w.r.t a = exp(aj-ai)*I(pi)
                    dp7[idx] = -hw * 1;  // w.r.t b
                }
                else {
                    if (ir_correction_model == 0) {
                        // ideal model
                        float drdA = (-rlR) + r2new_aff[3] * rlRPrev;
                        float dAda2 = 1 / r2new_aff[2] * (r2new_aff[0] - 1) * (-newFrame->ab_exposure);
                        dp6[idx] = hw * drdA * dAda2;
                        float drdB = -rlRPrev;//-hitColorPrev[0];
                        float dBdb2 = r2new_aff[1] * (prevFrame->ab_exposure - setting_frameTime);
                        dp7[idx] = hw * drdB * dBdb2;
                    }
                    else if (ir_correction_model == 1) {
                        // non-linear exponential model
                        dp6[idx] = hw * (-1) * 1;
                        dp7[idx] = hw * (-rlR) * r2new_aff[1] * (setting_frameTime - prevFrame->ab_exposure);
                    }
                    else {
                        // linear model
                        dp6[idx] = hw * (-1);
...
//	thisToNext.translation() *= factori;
    ...
    // JZ modification: previous frame
    void CoarseInitializer::setFirst(CalibHessian *HCalib, FrameHessian *newFrameHessian, FrameHessian *prevFrameHessian) {

        makeK(HCalib);
        firstFrame = newFrameHessian;

        // JZ modification: previous frame (res)
        firstPrevFrame = prevFrameHessian;
...
//
        ...
        makeNN();       // calculate the closet 10 neighbors of every point and their parent in the upper level
