...
// JZ modification: previous frame
    PointFrameResidual::PointFrameResidual(PointHessian *point_, FrameHessian *host_, FrameHessian *target_,
                                           FrameHessian *targetPrev_) :
            point(point_),
            host(host_),
            target(target_),
            targetPrev(targetPrev_) {
        efResidual = 0;
        instanceCounter++;
        resetOOB();
    ...
    double PointFrameResidual::linearize(CalibHessian *HCalib) {
        ...
        // JZ modification: previous frame
        const Eigen::Vector3f *dIlPrev = targetPrev->dI;
        ...
        const float *const colorPrev = point->colorPrev;
        const float *const weights = point->weights;
        // JZ modification: previous frame (previous exposure) of ref
        float refPrev_exposure = point->prev_ab_exposure;
        ...
        for (int idx = 0; idx < patternNum; idx++) {
            ...
            if (!IR_Data) {
                ...
                if (ir_correction_model == 0) {
                    ...
                    float hitColorPrevCoolTime = setting_frameTime - targetPrev->ab_exposure;
                    float refColorPrevCoolTime = setting_frameTime - refPrev_exposure;
            ...
            //std::cout << "hitColorPrev =  " << hitColorPrev[0] << "; colorPrev = " << colorPrev[idx] << std::endl;
            //std::cout << "affLL[0] = " << affLL[0] << "; affLL[2] = " << affLL[2] << "; affLL[1] = " << affLL[1] << "; affLL[3] = " << affLL[3] << "; residual = " << residual << std::endl;


            // JZ modification: affLight (gradient)
            // this now is using linearized points
            float drdA, drdB;
            if (!IR_Data) {
                drdA = (color[idx] - b0);     // JZ comments: shouldn't this be negative? (the sign matters)
                drdB = 1;
            }
            else {
                if (ir_correction_model == 0) {
                    // ideal model
                    drdA = color[idx] - affLL[3] * colorPrev[idx];//(1 / affLL[2]) * color[idx] - (1 / affLL[2]) * affLL[3] * colorPrev[idx]; //color[idx] - affLL[3] * colorPrev[idx]; //
                    drdB = colorPrev[idx];//hitColorPrev[0];//
                    //std::cout << "drdA = " << drdA << "; drdB = " << drdB << std::endl;
                }
                else {
                    // linear model and exponential non-linear model
                    drdA = 1;
                    drdB = colorPrev[idx];
                }
            }
            ...
            hw = fabsf(residual) < setting_huberTH ? 1 : setting_huberTH / fabsf(residual);
...

            {
                ...
                JabJIdx_10 += drdB * hw * hitColor[1];
                JabJIdx_11 += drdB * hw * hitColor[2];

                // JZ modification: affLight
                JabJab_00 += drdA * drdA * hw * hw;
                JabJab_01 += drdA * drdB * hw * hw;
                JabJab_11 += drdB * drdB * hw * hw;
...
//        Vec2f stepAB = J->JabJIdx * J->Jpdd;
//        std::cout << "JabJIdx_00 = " << JabJIdx_00 << "; JabJIdx_01 = " << JabJIdx_01 << "; JabJIdx_10 = " << JabJIdx_10 << "; JabJIdx_11 = "
//            << JabJIdx_11 << "; stepAB = [" << stepAB[0] << ", " << stepAB[1] << "]" << std::endl;

        if (energyLeft > std::max<float>(host->frameEnergyTH, target->frameEnergyTH) || wJI2_sum < 2) {
//            std::cout << "w = " << w << std::endl;
//            std::cout << "hw = " << hw << std::endl;
//            std::cout << "wJI2_sum = " << wJI2_sum << std::endl;
//            std::cout << "energy left = " << energyLeft << std::endl;
//            std::cout << "energy threshold = " << std::max<float>(host->frameEnergyTH, target->frameEnergyTH) << std::endl;
