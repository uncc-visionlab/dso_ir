...
namespace dso {
    ...
    PixelSelector::PixelSelector(int w, int h) {
        ...
        setting_pixelSelectionUseFast = true;
        allowFast = true;
        gradHistFrame = 0;
    }
...
//                //RGB
//                ///cv::Ptr<cv::SIFT> detector = cv::SIFT::create(0,8,0.02,15,1);   // (int nfeatures, nOctaveLayers, contrastThreshold, edgedThreshold, sigma)
//                //IR pt1
//                cv::Ptr<cv::SIFT> detector = cv::SIFT::create(0,8,0.01,20,1.2);  // for pinhole, 0, 6, 0.04, 10, 1
//                //IR pt2
//                //cv::Ptr<cv::SIFT> detector = cv::SIFT::create(0,8,0.02,20,1);
//                detector->detect(img8u, pts);
////                if(pts.size() < numWant) {
////                    std::vector<cv::KeyPoint> extraPts;
////                    cv::Ptr<cv::SIFT> detector2 = cv::SIFT::create(0,4,0.01,20,1);
////                    detector2->detect(img8u, extraPts);
