...
namespace dso {
    ...
    PointHessian::PointHessian(const ImmaturePoint *const rawPoint, CalibHessian *Hcalib) {
        ...
        memcpy(colorPrev, rawPoint->colorPrev, sizeof(float) * n);
        ...
        prev_ab_exposure = rawPoint->prev_ab_exposure;
    ...
    void FrameHessian::setStateZero(const Vec10 &state_zero) {
        ...
        // JZ modification: affLight (res) (how? dont understand)
        if (!IR_Data) {
            ...
            if (ir_correction_model == 0) {
                // ideal model
                assert(ab_exposure > 0);
                nullspaces_affine.topLeftCorner<2, 1>() = Vec2((1 - expf(-0.2f * ab_exposure)), expf(-0.2f * (setting_frameTime - ab_exposure)));
                nullspaces_affine.topRightCorner<2,1>() = Vec2(expf(-0.2f * (setting_frameTime - ab_exposure)), (1 - expf(-0.2f * ab_exposure)));
            }
            else if (ir_correction_model == 1){
                // non-linear exponential model
                nullspaces_affine.topLeftCorner<2, 1>() = Vec2(0, 1);
                assert(ab_exposure > 0);
                nullspaces_affine.topRightCorner<2,1>() = Vec2(expf(aff_g2l_0().a*ab_exposure), 0);
            }
        }
    };
...
// calculate the delta pose before and after optimization, as well as the brightness affine parameters and etc...
// JZ modification: previous frame  (frameHessiansPrev)
    void
    FrameFramePrecalc::set(FrameHessian *host, FrameHessian *hostPrev, FrameHessian *target, FrameHessian *targetPrev,
                           CalibHessian *HCalib) {
        this->host = host;
        this->target = target;

        // before optimization: this->worldToCam_evalPT = worldToCam_evalPT = newFrame->shell->camToWorld.inverse()
        SE3 leftToLeft_0 = target->get_worldToCam_evalPT() * host->get_worldToCam_evalPT().inverse();
        PRE_RTll_0 = (leftToLeft_0.rotationMatrix()).cast<float>();
        PRE_tTll_0 = (leftToLeft_0.translation()).cast<float>();


        // after optimization: PRE_worldToCam = SE3::exp(w2c_leftEps()) * get_worldToCam_evalPT()
        ...
        PRE_aff_mode = AffLight::fromToVecExposure(host->ab_exposure, target->ab_exposure, hostPrev->ab_exposure, targetPrev->ab_exposure,
                                                   host->aff_g2l(), target->aff_g2l(), IR_Data).cast<float>();
        PRE_b0_mode = host->aff_g2l_0().b;     // this won't be used in the current implementation
    }
